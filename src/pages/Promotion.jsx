...
const Promotion = () => {
  ...
  useEffect(() => {
    // Filtrer les produits en promotion (par exemple, avec une réduction > 10%)
    if (products.length > 0) {
      const promoProducts = products.filter(product => 
        product.originalPrice && 
        product.originalPrice > product.price && 
        ((product.originalPrice - product.price) / product.originalPrice * 100) >= 10
      ).slice(0, 12); // Limiter aux 12 premiers produits en promotion
      
      setPromotionProducts(promoProducts);
    }
  }, [products]);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {promotionProducts.map((product) => (
            <div 
              key={product._id} 
              className="bg-white rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl relative"
            >
